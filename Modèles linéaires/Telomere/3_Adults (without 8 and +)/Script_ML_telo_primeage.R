...
dantler_NA = dantler_NA[complete.cases(dantler_NA$RTL), ]
...
dantler_NA_without_AgeAccelResiduals = dantler_NA %>% 
  dplyr::select(Id, Year, Day, Age,
                Cohort, Population, 
                Weight, Antler_std,
                RTL) %>% 
...
reg_lm <- lm(RTL ~ Cohort + Population + 
                  Weight + Antler_std+
                  Antler_std:Cohort + Antler_std:Population + 
                  Weight:Cohort  + Weight:Population +
                  Cohort:Population
                ,  
                data=dantler_NA_without_AgeAccelResiduals)
...
reg_lm <- lm(RTL ~ Weight_log + Antler_std_log + Cohort + Population + 
